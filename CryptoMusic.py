...
def esteganografia():
    ...
    audio=input("Nome do arquivo WAV: ")

    segredo=escolher_arquivo()

    base=os.path.splitext(segredo)[0]
    ...
    if len(segredo_bytes)>len(audio_bytes):

        print("Arquivo muito grande para este áudio")

        return

    for i in range(len(segredo_bytes)):

        audio_bytes[i]=(audio_bytes[i]&254)|(segredo_bytes[i]&1)

    saida=base+"_ESTEG.wav"
